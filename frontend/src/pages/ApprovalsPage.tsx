...
export default function ApprovalsPage({ approvals, onChanged }: { approvals: Approval[]; onChanged: (items: Approval[]) => void }) {
  const [error, setError] = useState<string | null>(null);
  async function resolve(id: number, action: 'approve' | 'reject') {
    try { const updated = action === 'approve' ? await api.approve(id) : await api.reject(id); onChanged(approvals.map((item) => item.id === id ? updated : item)); }
