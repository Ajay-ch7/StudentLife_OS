  ...
  return (
    <div className="app-frame">
      ...
      <main className="app-shell">
        <header className="topbar"><div><p className="eyebrow">{view === 'dashboard' ? 'Workspace overview' : view === 'calendar' ? 'Schedule' : 'Student workspace'}</p><h1>{view === 'dashboard' ? `Good morning, ${profileName}` : view[0].toUpperCase() + view.slice(1)}</h1></div><div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}><button className="btn btn-secondary" onClick={() => setIsBriefingOpen(true)}>Morning Briefing</button><button className="btn btn-secondary" onClick={() => setIsInboxOpen(true)}>AI Extractor</button><div className="date-chip">{new Intl.DateTimeFormat(undefined, { weekday: 'short', month: 'short', day: 'numeric' }).format(new Date())}</div></div></header>
